...
const EncryptionModule = {
    decrypt: function(encryptedString, secretKey = process.env.ENCRYPTION_SECRET) {
        try {
            // กรณีที่เป็น JSON string ที่มาจากการเข้ารหัส AES
            ...
            // กรณีที่เป็น Base64 string ที่มาจากการเข้ารหัส XOR
                ...
                try {
                    // ถอดรหัส Base64
                    const base64Decoded = Buffer.from(encryptedString, 'base64').toString();
                    
                    // ถอดการสลับตำแหน่ง (unscramble)
                    const unscrambled = this.unscrambleString(base64Decoded);
    ...
    // ยังคงเก็บฟังก์ชัน parseSimpleFormat ไว้สำหรับ QR Code รูปแบบเก่า
    parseSimpleFormat: function (qrText) {
        try {
            if (qrText.startsWith("S:") || qrText.startsWith("STUDENT:")) {
                const parts = qrText.split(",");
                let studentId;
                
                if (qrText.startsWith("S:")) {
                    studentId = parts[0].split(":")[1];
                } else { // startsWith("STUDENT:")
                    studentId = parts[0].split(":")[1];
                }
                
                const timestamp = parts.find(p => p.startsWith("T:") || p.startsWith("TIME:"))?.split(":")[1];
                const expiry = parts.find(p => p.startsWith("E:") || p.startsWith("EXP:"))?.split(":")[1];

                return {
                    sid: studentId,
                    ts: parseInt(timestamp) || Date.now(),
                    exp: parseInt(expiry) || (Date.now() + 300000), // default 5 minutes
                    ver: "1.0",
                };
            }
            return null;
...
app.post("/api/process-qrcode", requireAuth, async (req, res) => {
  try {
    ...
    // ถอดรหัสข้อมูล QR Code หรือใช้ข้อมูลที่ถอดรหัสแล้วจาก client (ถ้ามี)
    let decodedData;
    try {
      // กรณีที่ข้อมูลถูกถอดรหัสมาแล้วจาก client
      if (studentInfo && studentInfo.studentId) {
        decodedData = {
          sid: studentInfo.studentId,
          name: studentInfo.name || "-",
          fac: studentInfo.faculty || "-",
          dep: studentInfo.department || "-",
        ...
        if (!decodedData) {
          console.error("ไม่สามารถถอดรหัส QR Code ได้:", qrData);
          return res.status(400).json({
            success: false,
