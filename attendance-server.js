...
app.post('/api/login', async (req, res) => {
  try {
    ...
    await pool.query(
      'SELECT log_action($1, $2, $3, $4)',
      ['login', 'ล็อกอินเข้าสู่ระบบสำเร็จ', user.id, req.ip]
    );
...
app.post('/api/logout', (req, res) => {
  if (req.session.user) {
    ...
    pool.query(
      'SELECT log_action($1, $2, $3, $4)',
      ['logout', 'ออกจากระบบสำเร็จ', userId, req.ip]
  ...
  try {
    ...
    if (checkAttendance.rowCount > 0) {
      ...
      await pool.query(
        ...
        ['มีการตรวจสอบซ้ำเมื่อ ' + now.toLocaleString('th-TH'), eventId, studentId]
      );
...
app.get('/api/reports/student-attendance', requireAuth, async (req, res) => {
  try {
    ...
    return res.status(500).json({
      success: false,
      message: 'เกิดข้อผิดพลาดในการดึงข้อมูลการเข้าร่วมของนักศึกษา กรุณาลองใหม่อีกครั้ง'
    });
  }
});
